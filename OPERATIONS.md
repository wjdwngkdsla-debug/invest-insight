...
- GitHub Actions가 매일 한국시간 08:40에 실행됩니다.
...
`IPO종목` 탭의 `수동공모가`는 선택 입력값입니다. DART 공모가 파싱이 실패했거나
잘못된 종목만 숫자로 입력하며, 빈칸이면 기존 저장값 또는 DART 자동 파싱값을 유지합니다.

`운영_락업일정`의 `상장주식수(YYYY-MM-DD)`와 예정/최종비중은 최근 거래일 KRX
상장주식수를 기준으로 매일 갱신됩니다. 락업 물량과 해제일은 다시 파싱하지 않습니다.
