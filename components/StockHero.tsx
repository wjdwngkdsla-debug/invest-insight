...
function ReturnGauge({ pct }: { pct: number }) {
  ...
  return (
    <svg viewBox="0 0 200 110" className="h-[92px] w-[164px]" aria-hidden>
  ...
  return (
    <div className="rounded-2xl border border-white/60 bg-white/55 px-4 py-3.5 shadow-[0_6px_24px_-8px_rgba(30,41,59,0.18)] backdrop-blur-xl">
      <div className="flex items-center gap-2">
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white/70 text-slate-500 shadow-sm">
          {icon}
        </span>
        <p className="break-keep text-[11px] font-medium leading-tight text-slate-500 sm:truncate sm:text-[11.5px]">{label}</p>
      </div>
      <p className="mt-2.5 break-keep text-[17px] font-bold leading-tight tracking-tight text-slate-900 sm:text-[18px]">{value}</p>
      <p className="mt-0.5 break-keep text-[10.5px] leading-[13px] text-slate-400 sm:h-[13px]">{sub || ""}</p>
    </div>
  );
}
...
export function StockHero({ stock, updated, initialNow }: { stock: StockLockup; updated: string; initialNow: number }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[32px] bg-slate-50 px-6 py-7 sm:px-8 sm:py-8">
      ...
      <div className="relative grid gap-5 md:grid-cols-[minmax(0,290px)_1fr] md:gap-6">
        {/* 좌: 배경 위에 그대로 얹은 종목 요약 */}
        <div className="flex flex-col justify-between gap-4">
          <div>
            <h1 className="text-[27px] font-bold leading-tight tracking-tight text-slate-900">{stock.name}</h1>
          ...
          <div className="flex items-end justify-between gap-1">
            <div className="min-w-0">
              ...
              {hasReturn ? (
                <>
                  <p className={`mt-1 text-[40px] font-bold leading-none tracking-tight ${up ? "text-rose-600" : "text-blue-600"}`}>
                    {up ? "+" : ""}
                    {changePct.toFixed(1)}
                    <span className="text-[22px]">%</span>
                  </p>
                  <p className="mt-2 whitespace-nowrap text-[11px] text-slate-500">
        ...
        {/* 우: 핵심 지표 6칸 — 2열 3행으로 좌측 히어로와 세로 높이를 맞춘다 */}
