...
def rows_to_site_data(
    ...
    for r in rows:
        ...
        stock = stocks_map.setdefault(code, {
            ...
            "listing_float_pct": _to_float(
                (listing_float.get(code) or listing_float.get(f"name:{managed_name(r.get('name'))}") or {}).get("float_pct")
            ),
            "listing_float_excludes_ipo_commitment": bool(
                (listing_float.get(code) or listing_float.get(f"name:{managed_name(r.get('name'))}") or {}).get(
                    "excludes_ipo_commitment"
                )
            ),
